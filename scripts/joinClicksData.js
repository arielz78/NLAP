...
const MIN_CELL_SIZE = 4;            // a percentile over 2-3 survivors is noise — drop the cell
...
// mid-rank percentile of each value within its cell, in [0,1]
function cellPercentiles(values) {
  const n = values.length;
  return values.map((v) => {
    let less = 0, eq = 0;
    for (const w of values) { if (w < v) less++; else if (w === v) eq++; }
    return (less + 0.5 * eq) / n; // n>=MIN_CELL_SIZE guaranteed by caller
  });
}
...
function main() {
  ...
  // ---- Cells + cell-size floor ----
  const cells = new Map();
  for (const p of eligible) {
    if (!cells.has(p.cellKey)) cells.set(p.cellKey, []);
    cells.get(p.cellKey).push(p);
  }
  let droppedSmallCells = 0;
  const labeled = [];
  for (const [, cell] of cells) {
    if (cell.length < MIN_CELL_SIZE) { droppedSmallCells++; continue; }
    const pctile = cellPercentiles(cell.map((p) => p.vuc));
    cell.forEach((p, i) => {
      labeled.push({
        section: p.section, slot: p.slot, slug: p.slug, issueDate: p.issueDate,
        url: p.url, vuc: p.vuc, vtc: p.vtc,
  ...
  fs.writeFileSync(outPath, JSON.stringify({
    ...
    config: { metric: CLICK_METRIC, maturationMinAgeDays: MATURATION_MIN_AGE_DAYS, minCellSize: MIN_CELL_SIZE },
    coverage,
    maturation: { minAgeDays: MATURATION_MIN_AGE_DAYS, excludedIssues: immatureIssues },
    cells: { kept: cells.size - droppedSmallCells, droppedTooSmall: droppedSmallCells },
    slotGradient,
  ...
  console.log("\n=== CELLS ===");
  console.log(`  ${cells.size - droppedSmallCells} cells kept, ${droppedSmallCells} dropped (<${MIN_CELL_SIZE} matched picks)`);
  console.log("\n=== SLOT GRADIENT (mean within-cell percentile by slot) ===");
