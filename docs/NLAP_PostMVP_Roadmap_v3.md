...
> **PRE-R6 ADDENDUM 2026-06-30 (do a scoping/research pass before opening R6) — captures the "are we doing scoring properly/professionally" question:**
> - **Pre-R6 research pass (do first, before the W4/W5 issues).** Run a deliberate research-mode pass (Claude.ai research mode + full context pack) to confirm the *approach* before building — R6 is plan-mode territory where the wrong method means rework. **Frame it as a method-*fit* question, not a method survey.** The risk is NOT too little sophistication — it's getting a web-scale ML answer dressed as "industry standard." Force the output to explicitly name **what would be over-engineering at this scale and why.** Multiple LLMs add little here (they converge on the same recommender-systems literature); the leverage is in the *framing*, so use a 2nd model only as a cheap cross-check on the conclusion, not a parallel survey.
> - **Problem classification (the anchor — have the research confirm or challenge this first).** This is **small-scale editorial ranking**, NOT a personalized web-scale recommender. The Indeed/Instagram/learning-to-rank reference class is the **trap, not the template**: those justify heavy ML via (a) billions of per-user implicit signals, (b) per-user personalization, (c) engagement-maximization objectives. NLAP has (a) CTOR-only feedback at low volume, (b) **one list to the whole audience** (no personalization), (c) editorial-quality objective judged against an editor's taste. Mis-classify the problem → every downstream recommendation is mis-scaled. These constraints *cap* the appropriate sophistication; they're a feature of the framing, not a limitation to engineer around. (Sharpens the 2026-06-04 "LLM/hybrid picker over weighted formula" call above.)
> - **Hard constraints to hand the research verbatim:** 1 newsletter, ~1 editor, weekly cadence, CTOR-only/low-volume feedback, no per-user data, hard per-segment quotas (5 slots), non-technical client handoff (must trust/override the scorer).
> - **Industry principles that DO scale down (the "do it properly" list — import these, skip the infra that doesn't):** (1) separate candidate-generation from ranking (already done: ingestion vs scoring); (2) **offline eval against the editor's historical picks** — `issue_history.json` = 72 issues of actual editor selections = a labeled backtest set; if the score's top-5 disagrees with the editor's historical picks most of the time, the *score* is wrong, not the editor (this is the R6-W4 backtest's real purpose); (3) **explainable scores** — a non-technical client must trust and override it, so a black-box learned ranker fails the handoff; transparent weighted score + LLM quality signal passes; (4) no leakage — don't score on fields absent at inference (the `LocationName` 0%-populated lesson → why #63 field sweep exists).
> - **Output of the pass = the R6 scope doc, including an explicit "heavier methods we are deliberately NOT using, and why" section.** That section *is* the professionalism — it proves the simplicity is a decision, not ignorance. (Method-fit discipline per CLAUDE.md: the judgment of when *not* to use a method is the skill.)
> - **2026-07-02 client-meeting reinforcement.** Asked the editor for his selection criteria directly; answer was thin/intuitive ("novelty, families, cheap" — and "families" is the *segment*, not a rank signal). Confirms the approach: **do not spec from stated criteria — ground the baseline in revealed preference** (his actual `issue_history.json` picks) and let the **backtest decide formula-vs-LLM** (a scorer that can't beat editor's-gut on the eval set = evidence the problem is LLM-shaped). Guard: don't build a scoring formula for the *appearance* of rigor. Captured 10 same-segment head-to-head pairs at the meeting (in `meetings/2026-07-02.md` §4.5) → additional labeled eval cases alongside the 72-issue backtest set.
