...
**Step status at a glance (2026-07-30).** Step 0 ✅ · **Step 1 LABELLED, QC IN PROGRESS — 239 of 239** (gate 89/89 · train 124/124 · walkthrough 12/12 · not-in-model-set 14/14). The repeatable one-pull audit still finds target conflicts, incomplete reasoning, link evidence not propagated into labels and one factual-provenance mismatch; the data is **not evaluation-ready** until those rows are adjudicated · **Step 2 CUT** (it duplicated Step 4a) · **Step 3 nearly done** — geography shipped (#109), date-window and completeness already existed in R1, language / is-it-an-event / cancellation dropped from scope; **one item left, the Eventbrite TLD allowlist, written in-repo and awaiting a re-import** · **Step 4a STAGED, NOT RUN** — plumbing built; pin 1 is set (`RECALL_DIAL = "global"`), pins 2–4 remain Ariel's, and `gate_step4a.py` must consume `NoneReason` using the live option strings and corrected target mapping before any final evaluation · Step 4 blocked on Step 1 QC · Step 5 untouched.
