...
4. **Editor sitting — IN PROGRESS (2026-08-07):** Instrument B complete; Instrument A **14/100**, next row A-1-15. B ran first as corrected top 8 unique series × 3 sections; A continues in randomized blocks. Calibration anchors were **omitted at packet build by protocol** (§90 records them as diagnostic; `sealed_answer_key.json` states the omission), so A is 100 rows, not ~110. Readout contract is §93: split A by the `LINK` marker, and exclude the corrected packet's 6 `gate_fit_overlap` rows from the transfer number.
