...
3. ~~**Build the disjoint Instrument A and B packets**~~ ✅ **DONE (2026-08-06, rebuilt under §92).** A = 100 series, B = 24 listings, disjoint at `record_id`, `unique_event_id` and `series_key`. **Pushed live 2026-08-07** into standalone Airtable table `tblLID8HlJpSmIpSz` ("R7 Live Audit (2026-08-13)"), 124 rows, readback-reconciled against the frozen `.jsonl` sheets with 0 missing / 0 extra and 0 rows pre-ruled. Blind sheets are in `.../editor_packets/` — **not** `editor_packets_superseded_row_quintiles/`.
4. **Editor sitting** — Instrument B first (top 8 × 3 sections), then Instrument A in randomized blocks (100 stratified). Calibration anchors were **omitted at packet build by protocol** (§90 records them as diagnostic; `sealed_answer_key.json` states the omission), so A is 100 rows, not ~110. Readout contract is §93: split A by the `LINK` marker, and exclude the 8 `gate_fit_overlap` rows from the transfer number.
