...
⚠️ **Corrected arithmetic — the removable rate is 29.9%, not the ~53% the population-correction paragraph below still runs on.** Eligibility on the complete gate slice is **66.8%** (n=184), superseding the 07-26 ~46.5% figure, which counted `outcompeted` rows as ineligible. **A perfect gate removes about a third of the pool, not half** — ranking is more load-bearing than planned, and **the measured junk-rejection points (0.95 → 43%, 0.90 → 55%) were computed on merged-None negatives and must be recomputed on this denominator before they price anything.** Numbers home: `NA/Vaughan_Metrics_Log.md`.
