...
Execution order only — **no status lives here.** State is the Status Snapshot below; rationale is `Decision_Log` §89 as amended by **§90**. Ordering rule: live gate validation precedes the product shadow, because the shadow observes only the accepted set and cannot measure what was wrongly rejected.

1. ~~**Freeze the live pull**~~ ✅ **DONE (2026-08-06).** The frozen validation pool is `data/tracking/snapshots/candidates_2026-08-06_0944.json` (3,465 records, captured 09:44 before the Stage-0 edit). **321 candidates in the Aug 14–23 window**, above the 125 floor and matching `depthCheck.js` exactly. **No offline Stage-0 correction was needed:** the pool carries **zero foreign-TLD Eventbrite rows in-window** (9 `.ca` + 3 `.com`, all allowlist-passing). The 37 foreign rows base-wide are all legacy — 26 arrived in May, 7 in June, 4 in March, **none in July or August** — because the §52/#59 venue-city geo gate had already closed the leak by a different mechanism than TLD matching. Control against a dead-source explanation: allowed-TLD Eventbrite kept arriving over the same period (67 in July, 11 in August). Step 1 therefore certifies **on measured evidence, not on a patch**.
2. ~~**Live scoring runner**~~ ✅ **DONE (2026-08-06).** Read-only; embedded the 321 survivors, fit the frozen-specified gate on the complete frozen training set, scored them, emitted gate + section probabilities. No retuning, no Airtable writes. Artifacts in `data/tracking/r7_live_runner/issue_2026-08-13/`. #125 closed 2026-08-07.
3. ~~**Build the disjoint Instrument A and B packets**~~ ✅ **DONE (2026-08-07, corrected during the sitting before B judgments).** A = 100 series; B = 24 unique series, eight per section, with repeat listings collapsed under the production one-series-per-section/week constraint. A/B remain disjoint at `record_id`, `unique_event_id` and `series_key`. Both regenerated batches replaced the unruled Airtable rows in `tblLID8HlJpSmIpSz`; blind sheets and sealed key in `.../editor_packets/` are the corrected canonical artifacts.
4. ~~**Editor sitting**~~ ✅ **DONE (2026-08-07, completed offline).** Instrument B **24/24**; Instrument A **100/100** — completion confirmed by Ariel 2026-08-13, after the 2026-08-07 9:27 PM log entry recorded it mid-sitting at 14/100. Rulings are in Airtable `tblLID8HlJpSmIpSz`; **no readout has been run** and no transfer number exists. B ran first as corrected top 8 unique series × 3 sections; A ran in randomized blocks. Calibration anchors were **omitted at packet build by protocol** (§90 records them as diagnostic; `sealed_answer_key.json` states the omission), so A is 100 rows, not ~110. Readout contract is §93: split A by the `LINK` marker, and exclude the corrected packet's 6 `gate_fit_overlap` rows from the transfer number.
5. **Read live transfer first; interpret Fork C only if it holds** (§90).
6. **Adjudicate Step 4c** on the rows the live evidence makes consequential → `R7_Step4c_Error_Mechanism_Table.md`.
7. **#108 offline arm** — isolated backfill-at-300, pass/fail frozen before embeddings (§89).
8. **Close W6.**

Steps 4–5 are the work. ⚠️ Not a one-session sequence: step 4 depends on editor availability, and step 2 requires a fresh voyage-4-large run over the live pool.
...
**Step status at a glance (2026-08-03, after Step 1c closed and the §75 diagnostic split).** Step 0 ✅ · **Step 1a ✅ CLOSED — 239 of 239 None rows labelled** · **Step 1b ✅ CLOSED (2026-07-31) — the §77 routing contract ships in both consumers** (`route_s77()` in `gate_step4a.py`, `targetOf()` in `auditR7Labels.js`), asserting one shared fixture `routing_s77_cases.json` · **Step 1c ✅ CLOSED (2026-08-03)** — round 1 returned 14 ELIGIBLE / 15 PERMANENT REJECT / 9 UNCLEAR; Ariel adjudicated the 9 UNCLEAR rows, the click-only rejections and the unsectioned ELIGIBLE rows, and **round 2 was cancelled without being issued**. All 38 dispositions settled as **25 gate-positive (Families 8 / Couples 10 / Golden 7) · 12 gate-negative · 1 withheld (r342, corrupt input)**; 37 rows written to Airtable and verified against a post-write refetch. `eval/step1c_reconciliation.json` records every row's verdict, verbatim reason, settled disposition, section, provenance and annotations. **16 of 38 dispositions are Ariel-adjudicated and 22 carry the editor's round-1 verdict — recorded per row, not hidden in an aggregate.** (The 16th is r175: its round-1 ELIGIBLE stood, but Ariel ruled over a conflicting first-sitting `Label`, so the disposition is his.) 11 rows outside the current model remain optional; r220 still withheld for language adjudication · **Step 2 CUT** · **Step 3 ✅ CLOSED (2026-08-06)** — the Eventbrite TLD allowlist is **live**. Hand-edited into the running `Clean/Filter1` node rather than re-imported, deliberately: the 2026-07-27 import of this same file reported success while deleting the manual trigger and renaming all 51 nodes (#111), and a full re-import to deliver nine lines was the wrong risk. Verified three ways — 19 boundary cases pass (`.ca`/`.com`/bare-host keep; `.de/.fr/.co.uk/.com.au/.nl/.at/.ch/.be/.sg` drop; `forms.gle` and non-Eventbrite hosts untouched; `noteventbrite.com` no false match), R1 ran clean afterwards, and a live-vs-repo comparison confirms 52 nodes / 51 connection keys / both Merge input counts intact. ⚠️ **Live's node names still carry the `1` suffix from #111 and the repo's do not** — cosmetic only, accepted knowingly (Ariel, 2026-08-06); `workflows/NLAP R1.json` was deliberately NOT overwritten from the live export, because the repo copy is the clean-named canonical one. 🚨 **Superseded 2026-08-27 — the divergence is no longer cosmetic.** Live's `BiblioCommons RSS Fetch1` was converted from an HTTP Request node to a **Code node** carrying an explicit Chrome TLS cipher list, because BiblioCommons' WAF fingerprints the handshake and 403s Node's default ordering (see `docs/source_decision_sheet.md` § BiblioCommons). The container also now runs `NODE_FUNCTION_ALLOW_BUILTIN=fs,https` (was `fs`) and `--restart unless-stopped`. **The committed `workflows/NLAP R1.json` still holds the httpRequest node proven to return 403, so a repo→live import of the current file would silently re-break BiblioCommons** — and the post-run suite would not catch it (`ingestion_rejections.jsonl` is still unwritten per #111, and three zero-contribution sources passed unremarked on the 08-27 run per #128). **`TODO(ariel)` before any write to that file:** either (a) let a live export overwrite it, accepting `1`-suffixed names as canonical and superseding the 08-06 call, or (b) keep the clean-named repo copy canonical and hand-add only the BiblioCommons Code node. Not decided; do not export, reconstruct, or import until it is. ⚠️ **#115's body was wrong about the live state**: it recorded that n8n still ran the old blocklist, when in fact live carried **no Eventbrite TLD filter at all** · **Step 4a ✅ DIAGNOSTIC RUN COMPLETE** — gate-slice AUC 0.889 on the pre-repair labels; no release bar taken from it · **Step 4b ✅ CLOSED** — 30 disagreement rows resolved (19 KEEP / 11 NEVER), all 11 corrections applied and verified in Airtable, and `eval/step4b_reconciliation.json` records the write · **Step 4c NUMERICAL RUN COMPLETE (2026-08-03) — NOT CLOSED** — the pin is flipped to `"4c"` and the fit ran under §85's pre-registered contract, preflight first. **Provisional operating point 0.4530**: gate-slice AUC 0.856, keeper recall 97.1% (101/104), junk rejection 47.5% (29/61), survivor precision 75.9%, Golden binding at 91.7% (22/24). Numbers home in `NA/Vaughan_Metrics_Log.md` — do not re-author them here. §75 reported descriptively as two class-separated cells (6/6 keyword-positives survived, 1/6 keyword-negatives rejected) with no automatic verdict and no influence on operating-point selection, per §88. **Not closed** because the fresh-30 readiness audit and the error-mechanism worksheet await Ariel's adjudication, but they no longer block product validation. ⚠️ **The entire 35-row worksheet is unadjudicated** — `R7_Step4c_Error_Mechanism_Table.md` carries `TODO(ariel)` on all 35 rows (29 unique CV groups), not only on the four the earlier wording named (r269, r288, r341/r97, r70). Per the Closing Sequence, step 6 adjudicates only the subset the live evidence makes consequential, not all 35. ⚠️ **#123 remains unresolved after the positive-side cutoff diagnostic:** the gate rejects 0 of 14 audience-narrowness negatives, while all 25 flagged cultural/religious or age/family positives survive at 0.4530 (6/6 and 19/19). The positive check establishes cutoff behaviour only; it does not prove score-distribution separability or whether the fix belongs in a feature, the labels, or ranking. A seven-group blind review showed that `open/restricted` is not equivalent to `keep/reject`; no canonical judgments or labels changed. **Decision_Log §90 (2026-08-06) amends §89: a live stratified validity audit now precedes the product shadow**, because the shadow observes only the accepted set and cannot detect eligible events the gate buried. See The Closing Sequence above for the execution order. #108 still follows as a 300-character backfill-only arm; any higher-cap arm is separate, and only an offline winner earns a second shadow. **Neither #108 nor the shadow changes live R1** — note this is now a scoped claim, not a blanket one: live R1's Stage 0 *was* changed on 2026-08-06 (Step 3, above). ⚠️ Run it with `models/.venv/Scripts/python.exe`, not `py -3` — there is no system Python on this machine · **NEXT: Closing Sequence step 5 — the sealed joined readout.** Step 4 (the editor sitting) closed 2026-08-07; A is 100/100 and B is 24/24. Split Instrument A by the `LINK` marker and exclude the 6 `gate_fit_overlap` rows before reading any transfer number (`Decision_Log` §93). (Execution order lives in The Closing Sequence above; the readout contract in §93.)
...
**#108 offline-arm preparation (2026-08-04): inventory and evidence preservation complete; comparison not run.** The fixed-path pre-adjudication Step-4c outputs are sealed under `models/sectioning/eval/sealed/step4c-pre-adjudication-2026-08-03/`, and `R7_AllEvents_Backfill_Inventory_2026-08-03.md` records model-set coverage, source-page artifacts and the global-cap confound. Per §89, the baseline shadow runs first. Then #108 tests recovered AllEvents descriptions at the unchanged 300-character cap; a higher-cap representation is a separate arm. Recipe approval, the frozen comparison/pass-fail contract, embeddings and evaluation remain open. No cached artifact, fit, threshold or live R1 behavior changed.

**#108 reach, measured 2026-08-04 (read-only, before §89's arm runs).** On the 165-row gate slice at the frozen point 0.4529508948326111, the backfill-only arm at cap 300 changes text on **40 rows** (21 previously title-only) but touches only **1 of 3 demoted keepers and 9 of 32 surviving junk** — the 9 collapsing to **6 unique listings** (r341/r97, r354/r62, r327/r47 are duplicate pairs). Ceiling if every touched error row flipped correctly: keeper recall 97.1% → 98.1%, junk rejection 47.5% → 62.3%. The separate higher-cap arm changes 24 of 165 rows between 300 and 600 (the same 24 to uncapped), 6 of them error rows including r218; only 11 rows exceed 600 characters, and a cap change would also require re-validating the section classifier — unverified. ⚠️ **Open and deliberately unrecorded (Ariel's explicit choice, 2026-08-04):** whether a winning enriched-text arm is deployable in W6 at all, since live R1 does not fetch AllEvents detail pages and `text_recipe.py` lines 36–41 commit the artifact rules to the same change that wires that fetch into R1. `R7_Cap_Signal_Position_Sheet.md` is staged, blind and unmarked; its 11 rows contain only 9 unique texts (r76/r320 and r86/r338 are byte-identical pairs), so any tally must not double-count them.
...
- **Arm order (§89):** run this baseline first on the current frozen representation. Afterward, run #108 offline as backfill-only at the unchanged 300-character cap, then test any higher-cap representation separately. Only an offline winner receives a second shadow packet.
...
The 07-09 feasibility capture, the dead representation gates, the 07-15→07-22 results log (probe numbers, confusion matrices, per-source None rates, self-consistency decomposition), and the now-superseded transfer-test / τ-fork investigation live there. Decisions in `docs/Decision_Log.md` §61–90.
