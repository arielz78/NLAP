...
**Step status at a glance (2026-08-01, after the Step 4b sitting).** Step 0 ✅ · **Step 1a ✅ CLOSED — 239 of 239 None rows labelled** · **Step 1b ✅ CLOSED (2026-07-31) — the §77 routing contract ships in both consumers** (`route_s77()` in `gate_step4a.py`, `targetOf()` in `auditR7Labels.js`), asserting one shared fixture `routing_s77_cases.json`; both print the gate slice as 95 positive / 54 negative · **Step 1c OPEN — 38 model-relevant `outcompeted` rows require blind relabelling; 11 outside the current model are optional; r220 remains withheld for language adjudication. It blocks 4c, not 4a** · **Step 2 CUT** · **Step 3 nearly done** — the Eventbrite TLD allowlist is written in-repo and awaiting re-import · **Step 4a ✅ DIAGNOSTIC RUN COMPLETE** — gate-slice AUC 0.889 on the pre-repair labels; no release bar taken from it · **Step 4b SITTING COMPLETE, CORRECTIONS PENDING** — all 30 disagreement rows have final judgments (19 KEEP / 11 NEVER / 0 unresolved); reconciliation and label writes are tracked in #121 · **Step 4c RULES PRE-REGISTERED AND IMPLEMENTED, RUN NOT STARTED** — §85's contract is enforced in code (section-safe threshold search, fresh-set exclusion against 4b's answer key, step-specific exports, behavioural precondition guard); it waits on #121's label work and #120's `SINGLE_COMMUNITY_PATTERN` provenance blocker · **Step 5 untouched and editor-dependent.**
...
- **Report both committed stratum diagnostics at the chosen point** (carried forward from Step 4, where they have sat unexecuted since 07-27): gate recall on the **single-community stratum** (§75's breadth un-park trigger) and on the **prof-dev / B2B stratum** (§76's content-judgment rows). Both are live in `gate_step4a.py` via `stratum_report()`. The B2B stratum is defined by the editor's own §77 reason tick, so it carries no invented vocabulary; the single-community stratum needs `SINGLE_COMMUNITY_PATTERN` authored, and **Step 4c refuses to run until it is** — the check has already survived three releases as a comment nobody executed.
