...
**Remaining Step 1c contract work:** the 14 ELIGIBLE rows have no editor-authored section and therefore no current route into the section-specific §85 veto; Ariel may selectively adjudicate clear cases with ABSTAIN available, but provenance and evaluation treatment must be explicit. Click-only and genuinely unclear cases remain withheld unless resolved. Separately, the §75 mixed-class diagnostic must be split or dropped before Step 4c. Step 4c remains on hold for those bounded items, tracked through #121.
