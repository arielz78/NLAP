...
**Step status at a glance (2026-08-02, after both non-editor 4c blockers closed).** Step 0 ✅ · **Step 1a ✅ CLOSED — 239 of 239 None rows labelled** · **Step 1b ✅ CLOSED (2026-07-31) — the §77 routing contract ships in both consumers** (`route_s77()` in `gate_step4a.py`, `targetOf()` in `auditR7Labels.js`), asserting one shared fixture `routing_s77_cases.json` · **Step 1c OPEN — ROUND 1 COMPLETE (2026-08-02): 14 ELIGIBLE / 15 PERMANENT REJECT / 9 UNCLEAR, recorded verbatim in `R7_Outcompeted_Editor_Form_38_Completed.md`. 12 rows queued for round 2, and 5 of the rejections turn on an unresolved design question (see the gate/ranker note below). No labels applied. 11 rows outside the current model are optional; r220 remains withheld for language adjudication. It blocks 4c, not 4a** · **Step 2 CUT** · **Step 3 nearly done** — the Eventbrite TLD allowlist is written in-repo and awaiting re-import · **Step 4a ✅ DIAGNOSTIC RUN COMPLETE** — gate-slice AUC 0.889 on the pre-repair labels; no release bar taken from it · **Step 4b ✅ CLOSED** — 30 disagreement rows resolved (19 KEEP / 11 NEVER), all 11 corrections applied and verified in Airtable, and `eval/step4b_reconciliation.json` records the write · **Step 4c RULES PRE-REGISTERED AND IMPLEMENTED, RUN NOT STARTED** — §85's contract is enforced in code; **#120 and #122 are both CLOSED (2026-08-02), so it now waits on Step 1c alone** · **Step 5 untouched and editor-dependent.**

⚠️ **THE GATE/RANKER BOUNDARY IS UNRESOLVED (2026-08-02). Do not treat any reading of it as settled.**

**What the evidence actually shows, and nothing more:** performance language appears in the editor's reasoning on **both sides** of the gate question. Five of Step 1c's 15 permanent rejections give *"doesn't get clicks"*; the Step 4b sitting shows the same language at a comparable rate (3/30 vs 5/38) but only on **KEEP** (*"it drives clicks"*, *"one of the best performing events"*).

⚠️ **An earlier draft of this line concluded *"the editor has one decision variable, not two — the gate/ranker split is our architecture, not his, and no prompt wording will get him to answer inside it."* That is WITHDRAWN** (Codex review, 2026-08-02). Eight comments across two sittings do not establish that the editor holds a single decision variable, and they do not establish that prompting cannot separate gate from ranker. Both remain untested hypotheses.

**The instrument finding is real but does NOT explain the click language — keep these separate.** The Step 1c form omitted both Step 4b repairs (no "assume the event is local" instruction, no position→URL lookup). Those omissions **do** explain r312 being answered on geography and six rows being answered "needs more information" with no link path. They **do not** explain the click-language leakage, which appears in Step 4b at a comparable rate *with* the repairs in place. An earlier draft attributed part of the click leak to the instrument; that attribution is also withdrawn.

**Sequencing:** the prior question is **how much gate/ranker fuzziness is acceptable by design** — an architecture decision, not yet made. A measurable sub-question exists beneath it (is low expected performance a property of the **event**, stable across appearances, or of the **week**? — answerable from Beehiiv history for types with published history), but it is deferred until the boundary question is settled. **Until then the five rows have no defensible target and Step 1c cannot be reconciled.**
...
**The stratum needs two numbers, not one:** **keyword-positive recall** (of the flagged rows the editor kept, what share survive the cutoff — should be high) and **keyword-negative rejection** (of the flagged rows the editor rejected, what share fall below it — compared against the slice's base negative-rejection rate). Only the second is the un-park test. **Unresolved: split it or drop the diagnostic from Step 4c entirely.** Not decided this session.

⚠️ `stratum_report()` currently returns `survival_rate` pooled over the whole stratum. It does print `n_pos` / `n_neg` so the mix is visible, but the headline number is the invalid one. **Resolve before Step 4c runs.** The B2B / prof-dev stratum is not affected in the same way — it is defined by the editor's own §77 reason tick and is therefore ~all-negative by construction, so its pooled rate is already a negative-rejection rate.
...
- **⛔ Step 1c is the ONLY remaining blocker.** Round 1 is complete (14 / 15 / 9) but **not reconciled and not applied**: 5 rejections turn on the unresolved gate/ranker question flagged in the Status Snapshot. Tracked under #121.
- **The 12-row round-2 queue is a PROPOSAL, not a decision.** Drafted 2026-08-02, **not sent to the editor**. Whether it is 12 rows, 6, or a different handling is open — three of the six "needs more information" rows answered *"depends on a week"*, which may already satisfy the ELIGIBLE condition and require no editor time at all. **Nothing was sent, no labels were applied, and Step 4c did not run.**
- **The §75 stratum diagnostic is unresolved** (see Step 4's corrected note) — its pooled reading is invalid and it must be split into keyword-positive recall and keyword-negative rejection, or dropped from 4c.
