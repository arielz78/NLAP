...
2. **Live scoring runner** — read-only; embeds survivors, fits the frozen-specified gate on the complete frozen training set, scores live survivors, emits gate + section probabilities. No retuning, no Airtable writes.
3. **Build the disjoint Instrument A and B packets** — no event appears in both.
4. **Editor sitting** — Instrument B first (top 8 × 3 sections), then Instrument A in randomized blocks (100 stratified + ~10 additional anchors).
