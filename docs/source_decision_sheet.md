...
- **Re-verified 2026-07-03 (folded into #63 sweep close-out):** `going.totalCount` no longer exists in the current API response — the field was removed upstream at some point since the 2026-06-23 audit; drop the "RSVP signal" note. `featured` is `true` on 100% of a 225-event sample — not a discriminating signal, contradicts the earlier "available, low priority" framing. **New fields found, not in the original audit:** `tickets` (`has_tickets`, `min_ticket_price`, `max_ticket_price`, `ticket_url` — cost signal, same class as Eventbrite/Pinot's/TRCA, sparsely populated: 2/225 in sample), `display_price_label` (AllEvents' own pre-formatted price string, e.g. `"CAD 30 to 49"`, `"Free"`), `tags` (free-text keywords, 170/225 populated, e.g. `["performances","nonprofit","charities"]`), `formats` (secondary category tag, e.g. `"festival"`, distinct from already-captured `categories[]`). All parked, no consumer yet — same cost-split rule as the rest of the #63 sweep.
...
- **Field audit ✅ complete (2026-07-03).** Full item tag set: `title`, `link`, `dc:creator`, `pubDate`, `guid`, `description`, `content:encoded`, `event_listing:location`, `event_listing:ticket_price`, `event_listing:organizer`, `event_listing:start_date`, `event_listing:end_date`. **New/uncaptured: `event_listing:ticket_price`** — not currently mapped; potential cost signal, park for R6 (cost-split rule: cheap field, could fold into DescriptionRaw later, no consumer yet).
...
- **Field audit ✅ complete (2026-07-03).** Per-card fields beyond title (`h3`) + date/time (`h4.time`, currently captured): `h4.price` (e.g. "$45 per guest"), painting image `src`/`alt` (the subject name, e.g. "Lunar Grove" — distinct from the class title in some cards), per-event detail link `/woodbridge/event/{id}` (booking page, not currently mapped as `Link` — current build uses the landing-page URL for all events), occasional bonus promo line (second `h4`, e.g. "Sangria Sunday - Pitchers of Sangria 10% Off!"). **New/uncaptured:** `price` (cost signal, same class as Eventbrite `ticket_availability`/BiblioCommons `capacity`), per-event detail URL (would let `Link` point at the specific class instead of the landing page — worth reconsidering if click-through data ever shows the landing page underperforms). Parked, no consumer yet.

**The Chef Upstairs — BookThatApp `schedule.json` (LIVE 2026-06-30 #80, T1 JSON)**
- **Field audit ✅ complete (2026-07-03).** Full occurrence keys beyond `title`/`start[]`/`handle`/`url` (currently captured): `id`, `end[]`, `allDay`, `bookingCount`, `remainingSpaces`, `capacity`, `capacity_type`, `variants[]` (`id`, `capacity`, `duration`), `variantBookingCounts[]`, `bookedOut`, `cutoff_time`, `vendor` (always "The Chef Upstairs"), `image`, `locations[]` (`id`, `name`, `timezone`), `resources[]` (empty), `tags[]` (e.g. `["Adult","Vaughan"]`), `backgroundColor`/`borderColor`/`textColor` (calendar-widget styling, no signal). **New/uncaptured:** `tags[]` — a real segment signal (Adult vs. presumably Kids/Family on camp entries), cheaper and more direct than DescriptionRaw inference; `remainingSpaces`/`capacity`/`bookingCount` — a genuine fill-rate/popularity signal, same class as Eventbrite `ticket_availability` and BiblioCommons `bc:registration_info`. Parked, no consumer yet — but `tags[]` in particular is a strong R6/R7 candidate given how cheap it is to capture (already in the feed, zero extra fetches).
