...
| **onrichmondhill.com** | **Ready (build-next)** — client-confirmed 06-24 | Drupal 7 RSS (T2) | `/?q=events/feed` (+ `/rss.xml`) | PASS — ~75% exclusive vs AllEvents RHill (#73), civic/community supply aggregators miss; **client confirmed build 2026-06-24**. Note: `/?q=events/feed` drifted to HTML since #73 — only `/rss.xml` is a real feed (re-verify at build) | ❌ open | PL 06-24 |
| **Richmond Hill** (city) | **Approved — probe pending** (client-confirmed 06-24) | calendar backend, method TBD | `calendar.richmondhill.ca` | **Client confirmed build 2026-06-24**, but **never probed** — integration method + effort unknown until probed. **Distinct from onrichmondhill.com** (#73); likely exclusive civic supply | n/a | §2b |
| **Pinot's Palette** | **Ready (build-next)** — client-confirmed 06-24 | per-venue scrape/calendar | `pinotspalette.com/woodbridge` | **BUILD** (client confirmed 2026-06-24) — featured **33×** in 72-issue history (#73 frequency check, top of cluster), Couples + Golden Age. **Partner relationship** — carries reader discount code `PinotsVaughan` ("10% off for Vaughan Brief readers"), so it's a commercial obligation, not just supply | ❌ open | PL 06-24 |
| **The Chef Upstairs** | **Ready (build-next)** — client-confirmed 06-24 | per-venue scrape/calendar | `thechefupstairs.com` | **BUILD** (client confirmed 2026-06-24) — featured **17×** in 72-issue history (#73 frequency check), Couples (mostly) + Families | ❌ open | PL 06-24 |
