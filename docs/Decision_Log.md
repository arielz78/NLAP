...
**Instrument A's transfer number excludes the gate-fit-overlap rows.** *(Added 2026-08-07 4:13 PM — before the sitting; verified zero rulings present in `tblLID8HlJpSmIpSz` at the time of writing.)* 8 of A's 100 rows carry `gate_fit_overlap = true`: the fitted gate saw those events during training, so their live score is partly in-sample. They are an **exclusion, not a comparison arm** — at n=8 the seen arm cannot carry its own transfer estimate, and reporting it beside the 92 as though it were a contrast would invite a difference the sample size cannot support. The split is recoverable after the fact by joining `sealed_answer_key.rows[].record_id` to `scored_survivors.jsonl`; verified as 8 of 100 A rows and 1 of 24 B rows, 0 unjoinable. **Provenance:** this constraint originates in the 2026-08-06 bounded review, not in a decision authored during this session; it is recorded here so that it sits on the record before any response exists rather than being reconstructed afterwards.
