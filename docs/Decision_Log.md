...
**Amendment (2026-09-03) — documentation is progressively specified.** Every maintained document records only verified current state, settled decisions, the next executable gate, and detail already required by a current consumer, build step or handoff. Unvalidated schemas, thresholds, feature lists, UI behaviour and work packages stay hypotheses or named gates until evidence and the next action require them. Parallel builders receive the minimum stable interface needed to avoid incompatible assumptions, not a speculative complete design. Full operating rule: `docs/README.md` § Progressive specification.

**Why:** the R6/R8 architecture review produced a coherent long-horizon design but initially translated it into a six-week specification during a five-day delivery window. Most of that detail would have needed rewriting before use. Progressive specification preserves direction without turning guesses into maintenance obligations; frozen logs still retain the full reasoning that produced each later decision.
...
**Amendment (2026-09-03) — parallel active releases also receive dedicated journals.** R6 ranking work, R8 delivery work and R7 closeout now proceed concurrently; forcing all three into `Execution_Log.md` would recreate the interleaving this decision exists to prevent. From the 2026-09-03 seam, R6 appends to `logs/R6_Log.md`, R8 appends to `logs/R8_Log.md`, and remaining R7 closeout plus release-neutral work stays in `Execution_Log.md`. These release journals are living while their releases remain open and freeze in place at close. Scope snapshots remain the only status source; journal location does not change ownership of open issues.
...
## 84. Blind Pre-Commitment Belongs to the Run That Sets the Bar, Not to Every Run (2026-07-31)

**Decision:** Step 4a pins **nothing** and reports **both** readings of every open measurement choice — global *and* per-section recall, gate slice *and* pooled fit, each reported separately. The four blind pre-commitments move to **Step 4c**, enforced by a `STEP` flag in `gate_step4a.py` that refuses to fit at `"4c"` until every pin is set. Pin 3 (`MERGED_NONE_VERDICT`) is **retired**; a fifth pin, the 4c saturation-check escalation trigger, is added.

**Why the guard was in the wrong place.** Blind pre-commitment exists to stop one specific failure: choosing the definition of "recall" *after* seeing which section sags, and thereby choosing the definition that flatters the model. That failure requires a number that becomes a **bar**. Step 4a no longer produces one — §82 made it diagnostic, and its job is to *locate label errors*, which is not a claim anyone can flatter. The guard was written when 4a set the bar and was never moved when that changed.

**What the reframe costs, and why it is not a loophole.** It would be one if the pins were then set from the diagnostic in a self-serving direction. The check is whether the informed choice runs *toward* or *away* from self-interest: `CURVE_MEASURED_ON = "gate"` selects the **worst** of the three populations (0.889 against pooled 0.910 and train 0.918). Nobody picks their least favourable slice to flatter a model. Those choices are therefore recorded as **informed at 4a**, with their provenance stated — not relabelled as blind.

**What is settled and what is not.** **One global threshold is chosen**; there will be no per-section thresholds. Per-section recall is reported as a **directional veto** — a signal to back off the *global* threshold, never to introduce a second one. ⚠️ **Whether it can support a numerical floor is unresolved**: Golden holds ~21 positives in the representative gate slice, so one event moves its recall ~4.8 points and any stated floor would be set by one or two events. The veto is directional until more representative labels exist. Do not write a per-section number into the Scope as a bar before that.

> **⚠️ SUPERSEDED IN PART by §85 (2026-07-31).** The paragraph above — "the veto is directional until more representative labels exist; do not write a per-section number into the Scope as a bar" — **no longer holds.** §85 writes a numerical floor (≥90% observed recall per section, in whole-event counts) *without* waiting for more labels, and states why. Everything else in §84 stands, including the retirement of pin 3 and the generalizable rule. Read §85 before acting on this paragraph.

**Why pin 3 is retired rather than answered.** It asked whether a **merged-None** curve sets the bar or only bounds it pending the four-way split. The split closed at 239/239 on 07-30 and the §77 routing contract replaced the merged target, so the question it guarded no longer has a referent. The live version of the same worry is not the negatives at all — it is the **unaudited positive class**, which is Step 4b's job and the `DIAGNOSTIC_STAMP`'s subject. **Same shape as score-vs-delete and as §82's own pin: the guard was still watching a door that had already shut, while the open one was behind it.**

**The generalizable rule:** *a pre-commitment is a function of what the run is allowed to decide, not of which script it lives in.* When a step's authority changes, its guards are re-derived from the new authority — otherwise they enforce yesterday's contract and read as rigour while doing nothing. The tell is a pin whose options no longer describe any live question.

---

## 85. Step 4c Uses a Representative-Slice, Section-Protected Dry-Run Readiness Contract (2026-07-31)

**Decision:** Step 4c keeps the embeddings-only logistic gate at **`C=1.0` with no release-time tuning** and reports gate, train and pooled curves separately. The representative **gate slice alone chooses one global reporting threshold**; train and pooled are diagnostics whose disagreement triggers investigation, never a switch to a friendlier denominator. The chosen point maximizes known-junk rejection subject to **at least 90% observed keeper recall in every section**, expressed in whole-event counts and recalculated after relabelling changes the denominators. There is no invented minimum junk-rejection percentage: the surviving point advances to the shadow dry run, where swaps and editor time measure operational value. Rows below the reporting cutoff are demoted, never deleted (§78).

**The positive-label readiness rule:** Step 4c audits a fresh, untouched 30-event disagreement set. **0–5 consequential target/destination flips** are corrected and permit progression; **6 or more** require a second targeted batch; a second batch also at 6 or more ends targeted sampling and triggers an audit of the remaining positive class. Any newly systematic failure mechanism pauses progression regardless of count. The set is deliberately enriched, so these counts estimate no population error rate — they price whether another targeted batch still buys enough repair to delay the end-to-end test.

**Why the section floor is a coarse veto, not precision.** The pre-repair representative positives split 46 Families / 28 Couples / 21 Golden. A single lost event therefore moves observed recall by about 2.2 / 3.6 / 4.8 points. At a 90% floor the current whole-event maxima are 4 / 2 / 2 below-cutoff keepers. The percentage supplies one consistent principle across unequal denominators; the event counts state what the evidence can actually resolve. It is a dry-run-readiness guard against one section absorbing the global loss, not an industry-standard recall claim or a population guarantee.

**This supersedes §84's "directional veto only — do not write a numerical floor" caution, and the resolution was not the one §84 anticipated.** §84 deferred the number until *more representative labels existed*. No additional representative labels arrived, and none are scheduled: Step 1c adds relabelled `outcompeted` rows and Step 4b repairs existing ones, but neither enlarges the representative gate slice. Waiting would therefore have deferred the floor indefinitely while W6 stayed blocked on it. **The choice made instead was to accept the coarseness explicitly rather than wait it out** — write the floor, translate it into whole-event limits (4 / 2 / 2) so nobody reads three-decimal precision into a 21-event denominator, and label it an **observed-slice veto** rather than a population guarantee. §84's objection is not answered by evidence; it is answered by narrowing the claim to what the evidence supports.

**The floor is boundary-hugging by construction, and that is why the dry run is load-bearing.** The chosen threshold is deliberately the *highest* threshold that still clears the floor, and it is selected on the same gate slice whose recall is then reported. So the binding section will sit just above 90%, not comfortably above it, and on a fresh sample it would fall below roughly as often as not. This is not a defect in the search — it is what "maximum junk rejection subject to the veto" means — but it does mean the reported section recall is the tightest passing value rather than a stable estimate. **Consequence: the shadow dry run is independent validation, not confirmation.** A pass there is the first evidence about the operating point that was not selected on the same data it is measured on, which is precisely why §85 refuses to require a minimum offline junk-rejection percentage before reaching it.

**Why curve movement cannot trigger escalation.** Step 4c follows label repair, so both the target and its denominators may change from Step 4a. The curve movement is reported descriptively but has no numeric trigger; treating it as clean before/after evidence would confuse a changed instrument with a changed model. Fresh label flips and newly systematic failure mechanisms govern escalation instead.
...
## 87. R7 Remains a Soft Viability Signal, R6 Owns Relative Ranking, and V1 Is Decision Support (2026-08-02)

**Decision:** V1 is a decision-support system: Stage 0 enforces deterministic exclusions, R7 produces a reversible viability signal, R6 owns relative weekly and click-oriented ranking, the allocator applies section and diversity constraints, and the editor retains final selection. This settles the gate/ranker boundary without redesigning R7-W6. The functions are logically distinct because they answer different questions and have different evidence; this does not commit the project to two learned models permanently. Until R6 ships, `P(include)` may contribute to interim ordering, but it is not interpreted as expected clicks or total event quality.

**Why neither imitation nor click optimization is the current target.** Editor labels are the strongest available eligibility evidence but mix audience judgment with some performance language and carry measured inconsistency. Historical engagement covers selected, published events and does not reconstruct how unselected weekly candidates would have performed, so it cannot yet train an unbiased raw-pool click ranker. Pure imitation would reproduce current judgment noise; pure click optimization would claim counterfactual evidence the project does not possess. Decision support uses the evidence each stage can defend while keeping uncertain candidates recoverable.
...
**Amendment (2026-09-03) — the delivery contracts are now explicit.** R6's directional output is a versioned, section-aware `RankedPool`; its exact schema remains deferred until the first consumer requires it. The current Airtable `Score_Final` cannot be that interface: it is an API-read-only formula equal to `Score_Manual`, whose populated values are zero. R6 must bypass it or explicitly migrate the field type. Clicks remain secondary evidence and a guardrail, not an unbiased selection target.

The allocator and W9 form one logical assembly boundary: ranking enters once; the boundary returns the collective five-event set per section, section-level alternatives, and contextual replacement assessments (`clean`, `override`, `unavailable`, with reason). This names a target contract, not an already-built component. `buildIssues.js` remains the sole production planner until a replacement runs read-only on identical inputs, records slate diffs, and cuts over atomically through `connectAirtable.js`; afterward the legacy planner is retired or wraps the shared implementation. Two planners may never write competing plausible slates.

This amendment does **not** settle the `RankedPool` schema, enrichment depth `M`, visible-alternative count `K`, exact scoring method, policy classification, or R8 presentation. Those details follow §45's progressive-specification rule.
...
## 92. Both Instruments Sample Series; Instrument B Initially Sampled Listings in Error (2026-08-06; corrected 2026-08-07)

**Decision:** both instruments draw **one row per recurring series**. A's rank-position quintiles are cut over the series population. B selects the top eight unique series per section, filling down past repeat listings, because production permits at most one occurrence of a recurring event in the same section/week. A series used in B is excluded from A in full, so no event is judged in both instruments. The series key is `gate_step4a.norm_title`'s convention — whitespace-collapsed lowercase title — so the packets and the gate's leak-free CV folds agree on what one event is.

**Why:** measured on the frozen 2026-08-13 pool, 321 in-window listings collapse to 225 distinct events. 41 groups are byte-identical in serve text and cover 134 listings; within those groups `p_include` varies by at most 1.2e-7 and the section argmax never differs. A repeat is therefore a duplicated editorial question. Instrument A requires unique series for statistical independence; Instrument B requires them because production already enforces one recurring event per section/week.

**Correction (2026-08-07, during the sitting and before B judgments):** the claim that production surfaces repeated occurrences within a section/week was false. Production already treats them as one editorial opportunity. The first B packet therefore violated the product constraint and was replaced before judgment. The gate, fit, section head and `0.4530` remain unchanged.
...
**Instrument B — quota-constrained shortlist quality, end to end.** B presents the top eight **unique recurring series** per section after applying production's one-series-per-section/week constraint (§92). The editor selects up to five events they would run. B is scored at section level on editor-approved events retained and replacements required to fill five slots—not as tie-broken listing agreement against a "model top five."

**Correction (2026-08-07, before B judgments):** duplicate-induced shortages in the original packet were an instrument-construction defect because they contradicted a known production constraint. The corrected packet contains eight unique series per section. The original packet remains recoverable through git history; it is not scored.

**Why B is deduplicated before presentation.** The editor ranks editorial opportunities, not repeated dates of the same program. Filling down to eight unique series measures the production shortlist; retaining repeats would spend judgments on a constraint already enforced elsewhere.

**Why there is no "sealed top five" for B.** The editor chooses up to five from eight unique ranked opportunities; B measures retained approvals and required replacements at section level. It does not manufacture a model top-five target that the editor is expected to reproduce.

**What this does not decide:** no pass bar. §90's "no pass bar at n=100" stands for A, and B's quantities are descriptive at 8 unique series × 3 sections. Nothing here changes the gate, fit, section head, `0.4530`, or frozen pool.
...
**Instrument A's transfer number excludes the gate-fit-overlap rows.** *(Added 2026-08-07 4:13 PM — before the sitting; corrected after the pre-judgment packet rebuild.)* The corrected A packet carries **6 overlap / 94 unseen** rows: the fitted gate saw those six events during training, so their live score is partly in-sample. They are an **exclusion, not a comparison arm**. The split is recoverable after the fact by joining `sealed_answer_key.rows[].record_id` to `scored_survivors.jsonl`; the packet rebuild changed the sampled rows, not the exclusion rule. **Provenance:** the rule originates in the 2026-08-06 bounded review; the 6/94 count comes from the corrected 2026-08-07 packet.
...
**Why:** two code paths make a naive write silently destructive, and they pull in opposite directions. `connectAirtable.js:218` `deleteUnlockedIssueItems()` deletes every unlocked IssueItem on current and future issues and rebuilds them from the allocator's own picks, so an ordinary swap disappears on the next allocation run while the newsletter still looks correct. The obvious remedy fails the other way: `generateBlurbs.js:270` filters `!i.lock`, so a row locked before copy is generated never receives `DisplayTitle`/`Description`/`CTA`, and `pushToBeehiiv.js:209` then throws on any item missing those fields — blocking the export until someone unpicks it. Only a controlled sequence avoids both, and placing that sequence outside the console keeps the dangerous semantics with the contributor who knows the pipeline.

**Correction (2026-08-28, bounded review, same session).** The first draft of this entry described failure 2 as silently rendering blank bullets. `pushToBeehiiv.js:113–117` does default those fields to `''` and `renderSlot()` does emit the bullet unconditionally, but the `main()` guard at line 209 makes that path unreachable — the failure is loud and blocking, not silent. **The decision is unchanged:** failure 1 is silent and destructive on its own and independently requires both the no-console-writes rule and the fixed apply/generate/lock order. Recorded rather than rewritten, because the overstatement was propagated to an external brief before it was caught. A genuinely silent third path does exist and is not yet covered: re-pointing an existing row's `Candidate` link leaves stale copy that passes the guard, so the export renders the previous event's blurb under the new event's URL. That belongs in the write contract.
