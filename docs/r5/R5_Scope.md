...
- **Reusability / config check (standing, every release — added 2026-07-02, audited 2026-07-03).** R5 closes only after confirming **nothing city-specific leaked outside config** — the pipeline stays **city-swappable (base-per-newsletter)**. Audit result: **reusability: partial gap found, deferred to R6 with a recorded map (not silently assumed clean).**
  - **Clean already:** `airtableBaseId` and `beehiivPubId` are not hardcoded anywhere in the live workflow's Code nodes — they're handled via n8n credentials/connection config, which is the correct portable pattern. No gap here.
  - **Gap found:** there is no live config/Set node. Geo/city constants (`Vaughan`, `Markham`, `Richmond Hill`, `Woodbridge`, `Maple`, `Kleinburg`, `Thornhill`) are duplicated inline across **17 Code nodes** in the live `NLAP R1` workflow: `Make UniqueEventID`, `Eventbrite Normalize`, `McMichael Normalize`, `TRCA RSS Normalize`, `BiblioCommons Normalize`, `AllEvents Normalize`/`Normalize2`/`Normalize3`, `VisitVaughan Normalize`, `Unionville Normalize`, `Facebook Parse`, `VPL Normalize`, `OnRichmondHill Normalize`, `RichmondHill Parse Feed`, `RichmondHill Normalize`, `Pinot's Normalize`, `Chef Normalize`. A Mississauga clone today means hand-editing all 17, not swapping one file.
  - `docs/r5/vaughan.r5-w1.config.json` was corrected 2026-07-03 to match the current 11-source reality (was stale since W1) and marked `_status: SNAPSHOT ONLY` — it is not wired to the pipeline.
  - **Why not extracted now:** R6 hasn't decided formula-vs-LLM scoring yet — the config's shape (weights vs. prompts/eval-sets) depends on that decision. Extracting city constants into a Set node now, before R6 locks the scoring config's shape, risks doing the wiring twice. Building full swap-infrastructure also has no current consumer — Mississauga isn't scheduled (R8, per roadmap).
  - **Disposition:** extraction bundles into **R6**, alongside the scoring-weights config-drive work R6 already has to do on several of these same nodes (one careful pass touching each node once, not two separate live-editing passes). This is a standing convention every Scope doc copies (`docs/README.md`), so it can't become an R8 afterthought: **R6** — scoring weights + venue/segment logic AND the geo/city constant extraction above, together, config-driven, not Vaughan-baked; **R7** — separate model per base; **R8** — full onboard-via-config verified. This is *config-templating*, not true multi-tenancy — don't build tenant-isolation infra.
