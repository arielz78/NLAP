...
- **Milestone-completeness check (added 2026-06-21).** R5 closes only when **every issue in the R5 GitHub milestone is either closed or explicitly deferred with a recorded disposition** (deferral reason + where it lands). The milestone — not the `r5` label — is the accounting unit (GitHub's % counts the milestone). This requires issue/milestone hygiene to stay clean (#68); it lives here in the Scope doc, never in the roadmap (frozen intent ≠ status). Run `gh issue list --milestone R5 --state open` at sign-off and disposition each.
