...
- **Reusability / config check (standing, every release — added 2026-07-02).** R5 closes only after confirming **nothing city-specific leaked outside config** — the pipeline stays **city-swappable (base-per-newsletter)**: the newsletter config carries every city-specific field (baseId, pubId, scoringWeights, prompts, sources, geo-filter), and no Vaughan value is hardcoded into shared logic. This is a standing convention every Scope doc copies (`docs/README.md`), so it can't become an R8 afterthought: **R6** — scoring weights + venue/segment logic must be config-driven, not Vaughan-baked; **R7** — separate model per base; **R8** — full onboard-via-config verified. At each release sign-off, explicitly record **"reusability: done"** (or the specific gap deferred, with where it lands). This is *config-templating*, not true multi-tenancy — don't build tenant-isolation infra.
