...
10. **Editor-participation risk — unmitigated.** Swap-rate (the §4 gate) only produces data if the editor actually engages the ranked curation view weekly; R5 already flagged editorial throughput as the binding constraint (~99% of supply sits at New). Nothing in this plan secures that behavior — it's a client/process conversation, not a build item. Raise at the next client meeting: agree the weekly review loop before R6 ships, or the gate is unmeasurable.
11. **Measure the imitability ceiling.** The editor's self-consistency bounds what any imitator can reach (§4.3), but it's unmeasured. Cheap probe: re-ask ~3 of the 10 head-to-head pairs at a later meeting, unannounced; his agreement with his own earlier answers estimates the ceiling. Two minutes of editor time; sets the realistic swap-rate target before the gate is judged against an imaginary 100%.
12. **On finalization:** critique pass (§58) → replace this DRAFT banner → Decision_Log entry for §3 → open `r6` issues (build items, #69, config extraction bundle) → **#82 before touching shared nodes**. The critique pass runs **blind** — do not feed it the author's self-review (anchoring turns an independent check into confirmation); reconcile the two lists *after* it returns.
13. **Historical keep-vs-reject backtest (small-N, session-scoped).** Distinct from #6's *forward* pool-minus-picks: a genuine *historical* adjudication exists in Airtable from the editor's one real curation session — **3 clustered days (2026-05-28/30/31), ~20 approvals + ~14 rejects**, isolated by scoping `Status`-change timestamps to those days (the Apr-11 batch is bulk-enrichment eligibility junk — B2B/civic — and must be excluded; only session-day rejects are valid consumer events he passed over). Directional check: does a candidate score rank the ~20 keeps above the ~14 passes, within segment? **N≈34 — a sanity gutcheck, not a training set or a gate.** Recovery gotcha: the field is `StatusLastModified ` with a **trailing space** (clean-name queries 422; see Decision_Log §27 2026-07-12 note + GH field-hygiene issue). Provenance: this is the only true keep-vs-reject signal that exists; issue_history gives ordering-of-keeps only, not selection.
